...
def PrintClassFor(descriptor, msg_desc, out):
  ...
  if msg_desc.name in ['ValidationResult', 'ValidationError']:
    out.append(' * @export')
  out.append(' */')
  ...
  for field in msg_desc.fields:
    if field.label == descriptor.FieldDescriptor.LABEL_REPEATED:
      ...
      out.append(f'  this.{UnderscoreToCamelCase(field.name)} = [];')
    else:
      out.append('  /** @export {?%s} */' % FieldTypeFor(descriptor, field))
      out.append(f'  this.{UnderscoreToCamelCase(field.name)} = null;')
  out.append('};')
  out.append('')
...
def GenerateValidatorGeneratedJs(specfile, validator_pb2, text_format,
  ...
  out.append(' */')
  out.append(f'{rules_obj} = createRules();')
